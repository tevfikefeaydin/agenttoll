...
function validCore(row: Row): boolean {
...
/** Aggregate only supplied, allowlisted browser-inspection request logs. */
export function summarizeInspectionLogs(input: string | readonly string[], additionalOperatorWallets: readonly string[] = []) {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (group) {
      ...
      group.relevant ||= validCore(row);
    } else requestGroups.set(id, { row, fingerprints: new Set([fingerprint]), records: 1, relevant: validCore(row) });
  }
  for (const group of requestGroups.values()) {
    ...
    if (!validCore(group.row)) continue;
    rows.push(group.row);
  ...
  for (const row of rows) {
    ...
    const fingerprint = `${payer}/${amount}/${delivery}`;
    const observedAt = Date.parse(row.t as string);
    const receipt: Receipt = { fingerprint, network, payer, amount, observedAt,
  ...
  return {
    ...
    scope: {
      ...
      description: 'Supplied application log records for the allowlisted browser inspection client only.',
    },
    window: {
