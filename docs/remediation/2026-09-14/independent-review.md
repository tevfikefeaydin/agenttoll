...
## Incremental review: warning for the audited legacy npm version

**GO for `.github/workflows/warn-legacy-mcp.yml`; no material finding.** The workflow is manual, checks out its workflow commit and has read-only repository permissions. Its only npm metadata write targets the fixed `agenttoll-mcp@0.13.0` version, with no range or user-supplied version/message. The existing NPM secret is available only to that single deprecate step. Before the write, the job independently installs/tests explicit public 0.14.0 and requires its archive hash to match the recorded CI release integrity. Public confirmation afterward is a GET-only loop with a five-minute budget and requests capped at 15 seconds or the remaining budget. The ten-minute job timeout also bounds the complete workflow. There is no automatic repeat of the metadata write.

The notice's assertions match the saved original 0.13.0 audit: the documented session budget, trusted recipient, registered endpoint price and cancellation controls were absent, while 0.14.0 provides those controls and the keyless/23-tool behavior. The wording does not generalize to unaudited older versions or claim that the SDK has no independent amount limit. Upgrade/restart guidance is explicit. The [official npm deprecate documentation](https://docs.npmjs.com/cli/v11/commands/npm-deprecate/) confirms this operation adds registry warning metadata and can be reversed with an empty message; it does not remove the package or alter an already running installation.

Independent checks parsed the YAML, verified the sole manual trigger, fixed exact-version command, replacement/integrity ordering, single secret-scoped write and bounded read-only confirmation, and syntax-checked the extracted inline JavaScript. All passed. The reviewer did not execute `npm deprecate`, obtain npm credentials, publish a version or sign a payment. Actual workflow success and the public notice remain pending evidence; no claim is made that existing clients have upgraded or will restart automatically.
